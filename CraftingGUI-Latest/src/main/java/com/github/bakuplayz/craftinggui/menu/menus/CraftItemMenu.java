...
public final class CraftItemMenu extends DefaultMenu {
    ...
    private void craftItem(int amount, boolean isMaxCraft) {
        craftUtil.removeIngredients(playerInventory, amount);
        if (isMaxCraft) updateCanCraftCache(amount);
        addResults(amount);
    }
    ...
    private void addResults(int amount) {
        for (int i = 0; i < amount; i++) {
            ...
            if (VersionUtil.isInInterval(14.9, 100) && result.getType() == Material.HONEY_BLOCK) {
                playerInventory.addItem(new ItemStack(Material.GLASS_BOTTLE, 4));
            }
    ...
    private boolean canCraftItem(int amount) {
        return !recipeItem.isBanned()
                && craftUtil.containsIngredients(amount)
