...
public final class TableChoiceMenu extends DefaultMenu {
    ...
    public void handleMenu(final @NotNull InventoryClickEvent event) {
        ...
        if (clicked.equals(getMultiChoiceItem())) {
            playerSettings.setTableChoice(player, "Multi");
            updateMenu();
        }

        if (clicked.equals(getTableChoiceItem())) {
            playerSettings.setTableChoice(player, "Table");
            updateMenu();
        }

        if (clicked.equals(getDefaultChoiceItem())) {
            playerSettings.setTableChoice(player, "Default");
            updateMenu();
        }
    ...
    private @NotNull ItemStack getMultiChoiceItem() {
        boolean hasMultiSelected = playerSettings.getTableChoice(player).equals("Multi");
    ...
    private @NotNull ItemStack getTableChoiceItem() {
        boolean hasTableSelected = playerSettings.getTableChoice(player).equals("Table");
    ...
    private @NotNull ItemStack getDefaultChoiceItem() {
        boolean hasDefaultSelected = playerSettings.getTableChoice(player).equals("Default");
