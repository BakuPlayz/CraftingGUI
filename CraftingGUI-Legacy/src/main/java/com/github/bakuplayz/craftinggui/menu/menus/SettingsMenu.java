...
public final class SettingsMenu extends DefaultMenu {
    ...
    private @NotNull ItemStack getColorPickerItem() {
        short selectedColor = playerConfig.getSelectedColor(player);
    ...
    private @NotNull String getMultiChoice() {
        ...
        String playerChoice = playerConfig.getTableChoice(player);
        if (playerChoice.equals("Multi")) return LanguageAPI.Menu.MULTI_CHOICE.getMessage(plugin);
        if (playerChoice.equals("Table")) return LanguageAPI.Menu.TABLE_CHOICE.getMessage(plugin);
