...
public final class MultiChoiceMenu extends DefaultMenu {
    ...
    public void handleMenu(@NotNull InventoryClickEvent event) {
        ...
        if (clicked.equals(getMultiChoiceItem())) {
            playerConfig.setMultiChoice(player, "Multi");
            updateMenu();
        }

        if (clicked.equals(getTableChoiceItem())) {
            playerConfig.setMultiChoice(player, "Table");
            updateMenu();
        }

        if (clicked.equals(getDefaultChoiceItem())) {
            playerConfig.setMultiChoice(player, "Default");
            updateMenu();
        }
    ...
    private @NotNull ItemStack getToggleItem() {
        ...
        return new ItemUtil(Material.STAINED_CLAY)
                ...
                .setDamage(isEnabled ? 5 : (short) 14)
                .toItemStack();
    }

    private @NotNull ItemStack getMultiChoiceItem() {
        boolean hasMultiSelected = playerConfig.getMultiChoice(player).equals("Multi");
    ...
    private @NotNull ItemStack getTableChoiceItem() {
        boolean hasTableSelected = playerConfig.getMultiChoice(player).equals("Table");
    ...
    private @NotNull ItemStack getDefaultChoiceItem() {
        boolean hasDefaultSelected = playerConfig.getMultiChoice(player).equals("Default");
